...
void AbstractPaperTrading::batchPlaceOrder(const NewOrderList &orders, ResultList &result) {
	result.clear();
	for (const auto &x: orders) {
		try {
			json::Value y = placeOrder(x.symbol, x.size, x.price, x.client_id, x.replace_order_id, x.replace_excepted_size);
			result.push_back({y, json::Value()});
		} catch (std::exception &e) {
			result.push_back({nullptr, json::Value(e.what())});
		}
	}
}
