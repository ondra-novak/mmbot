...
bool HttpAPI::post_api_config(Req &req, const Args &v) {
	bool history = v["history"] != "false";
	req->readBody(req, max_upload) >> [me = PHttpAPI(this), history](Req &req, std::string_view text) {
		auto auth = me->core->get_auth().lock();
		auto user = auth->get_user(*req);
		...
		try {
			...
			if (history) {
				PStorage &hist = me->core->get_history();
				JSONHistory jhst(hist->load(), me->core->get_hist_size());
				jhst.add_rev(cfg, std::time(nullptr), json::Value());
				hist->store(jhst.get());
			}
...
bool HttpAPI::get_api_config_history(Req &req, const Args &v) {
	if (!check_acl(req, ACL::config_view)) return true;
	JSONHistory hist(core->get_history()->load());
	json::Value curcfg = core->get_config();
	json::Array res;
	hist.enum_revisions([&](std::size_t idx, json::Value sum) {
		json::Value diff = idx == 0?make_JSON_diff(curcfg, sum["c"]):sum["c"];
...
bool HttpAPI::get_api_config_history_id(Req &req, const Args &v) {
	if (!check_acl(req, ACL::config_view)) return true;
	JSONHistory hist(core->get_history()->load());
	std::string_view id = v["id"];
	auto h = std::strtoull(id.data(), nullptr, 10);
	std::size_t found(-1);;
	hist.enum_revisions([&](std::size_t idx, json::Value sum) {
		if (sum["h"].getUIntLong() == h) found = idx;
	});
	if (found == std::size_t(-1)) {
		api_error(req, 404, "Revision not found");
	} else {
		send_json(req, hist.get_rev_by_index(found));
	}
	return true;
}
