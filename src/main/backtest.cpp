...
#include "istatsvc.h"
#include "main/alert.h"
#include "main/istrategy.h"
#include "main/strategy.h"
#include "mtrader.h"
#include "sgn.h"
...
BTTrades backtest_cycle(const MTrader_Config &cfg, BTPriceSource &&priceSource, const IStockApi::MarketInfo &minfo, std::optional<double> init_pos, double balance, bool neg_bal, bool spend) {
	...
	try {
		...
		for (price = priceSource();price.has_value();price = priceSource()) {
            ...
            ord_price = p;		
            double cur_price = bt.price*0.9+p*0.1;
            Strategy::OrderData order = s.getNewOrder(minfo, cur_price, 2*cur_price-p, -dir, pos, adjbal,false);            
            ord_price = order.price?order.price :2*cur_price-p;
            if (ord_price *dir > cur_price * dir || (-dir)*order.size < minfo.calcMinSize(ord_price)) {
                order = s.getNewOrder(minfo, cur_price,p, dir, pos, adjbal,false);            
                ord_price = order.price?order.price :2*cur_price-p;
            } else {
                dir = -dir;
            }
			...
			minsize = minfo.calcMinSize(prev_price);
