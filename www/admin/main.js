"use strict";
var langobj = new Lang();
var avail_langs = [
	["en","data:image/png;base64,"],
	["cs","data:image/png;base64,"]
];

async function formTest() {
	var def = await (await fetch("/api/admin/forms")).json();
	var form = create_form(def.trader, "trader", "init");
	form.open();
}

var page_root;

function chooselang_dlg() {
	return new Promise((ok)=>{
		let dlg = create_form(avail_langs.map(x=>{
			return {
				"bottom":true,
				"type":"imgbutton",
				"src":x[1],
				"name":x[0]				
			}}),"choose_lang","","Choose language");				
		avail_langs.forEach(x=>{
			let l = x[0];
			dlg.setItemEvent(l,"click",async ()=>{
				await langobj.load(l);
				dlg.close();
				localStorage["lang"] = l;
				ok(l);						
			});	
		});
		dlg.openModal();
	});
}

async function chooselang() {
	const lang = localStorage["lang"];
	if (!lang) {
		chooselang_dlg();
	} else {
		return langobj.load("cs");
	}
}

async function start() {
	
	await chooselang();
	
	page_root = load_template("root");
	document.body.appendChild(page_root.root);
	
	
	
	
	
	
	
}