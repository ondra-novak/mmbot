...
App.prototype.fillForm = function (src, trg) {
	...
	data.shg_boostmode={value:"30"};
	data.shg_boost_custom=1;
	data.shg_r=0;
	...
	if (data.strategy == "halfhalf" || data.strategy == "keepvalue" || data.strategy == "hypersquare") {
	...
	} else if (data.strategy == "DCAM") {
        data.dcam_type = filledval(src.strategy.fn_type, "value_sinh");
        data.dcam_budget = filledval(src.strategy.initial_budget, 0);
        data.dcam_pown = filledval(src.strategy.power, 25);
		...
		data.trnd_fast = filledval(src.strategy.fast, data.trnd_fast);
...
function getStrategyData(data, inv) {
	...
	if (data.strategy == "halfhalf" || data.strategy == "keepvalue" || data.strategy == "hypersquare") {
        ...
        strategy  ={
            type: data.strategy,
            fn_type: data.dcam_type,
            initial_budget:data.dcam_budget,
            initial_yield_mult:data.dcam_iym,
            multipler:data.dcam_mult,
            power:data.dcam_pown,
		...
		strategy = {
			...
			custom_spread:data.shg_boost_custom,
			ratio: data.shg_r
		};		
	} else 	if (["hyperbolic","linear","sinh","sinh_val","sinh2"].indexOf(data.strategy) != -1) {
		strategy = {
...
App.prototype.init_backtest = function(form, id, pair, broker) {
	...
	var inputs = ["strategy","external_assets", "acum_factor","kv_valinc","kv_halfhalf","min_size","max_size","position_offset","linear_suggest","linear_suggest_maxpos",
		...
		"trnd_bip","trnd_ema","trnd_ech","trnd_p","trnd_rst","trnd_fast","trnd_lim","trnd_hst",
